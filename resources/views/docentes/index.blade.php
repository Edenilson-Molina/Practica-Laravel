        ...
        <section class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <article class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex flex-col p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <section>
                        <h2 class="font-semibold">Responsabilidad de docentes</h2>
                        @foreach ($docentes as $docente)
                            <div class="flex flex-row justify-between">
                                <div>
                                    <p>{{ $docente->dui_docente }} | {{$docente->nombres_docente}} | {{$docente->apellidos_docente}}</p>
                                </div>
                                <div>
                                    <a href="{{route('docentes.edit',$docente)}}" class="text-blue-600">Editar</a>
                                    <form action="{{route('docentes.destroy', ['docente'=> $docente])}}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
